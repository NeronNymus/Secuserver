...
def tcp_client(target_host, target_port, local_host, local_port):
    ...
    try:
        ...
        try:
            banner = aes_encrypt(aes_key, banner.encode())
        except Exception as e:
            pass
        ...
        while True:
            ...
            print("FLAG: Waiting for instructions")
            instructions = client.recv(4096)
            try:
                decrypted_instructions = aes_decrypt(aes_key, instructions)
                print(f"[!] Decrypted instructions:\t'{decrypted_instructions}'")
            except Exception as e:
                continue
